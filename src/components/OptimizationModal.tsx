            ...
            <div className="flex justify-between items-center mb-4">
                ...
                {bestResult && !isLoading && (
                    <button 
                        ...
                        className="px-3 py-1.5 bg-green-600/30 hover:bg-green-600/50 text-green-300 text-xs font-semibold rounded-md transition-colors flex items-center gap-2"
                    ...
                    <table className="w-full min-w-full text-sm text-left">
                        ...
                        <tbody>
                            {results.map(res => {
                                ...
                                return (
                                    <tr key={res.value} className={`border-b border-brand-border/50 last:border-b-0 ${isBest ? 'bg-green-500/10' : ''}`}>
                                        <td className={`p-2 font-mono font-semibold ${isBest ? 'text-green-300' : ''}`}>{res.value.toFixed(isInt ? 0 : 2)}</td>
                                        ...
                                        <td className={`p-2 text-right font-mono font-bold ${isBest ? 'text-green-300' : ''}`}>{res.results.sharpeRatio}</td>
