...
const OptimizationModal: React.FC<OptimizationModalProps> = ({ isOpen, onClose, parameterKey, parameterTitle, currentValue, onRun, onApply, results, isLoading }) => {
  ...
  const handleRun = () => {
    ...
    if (Object.keys(newErrors).length === 0) {
        ...
        if (steps > 50) {
          newErrors.step = "Range / Step results in too many iterations (max 50).";
          newErrors.end = "Range / Step results in too many iterations (max 50).";
        }
          ...
          <section className="w-full md:w-2/3 p-6 flex flex-col min-h-0">
            ...
            <div className="flex-grow overflow-y-auto">
                {isLoading && results.length === 0 ? (
                    ...
                    <div className="flex items-center justify-center h-full text-brand-muted">Define a range and click "Run Optimization".</div>
                ) : (
