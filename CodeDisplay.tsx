...
const CodeDisplay: React.FC<CodeDisplayProps> = ({ code, isEnabled, onClose, isModal, onFullscreen }) => {
  ...
  return (
    <div className={`bg-brand-secondary border border-brand-border rounded-lg flex flex-col relative ${isModal ? 'max-h-[90vh]' : 'h-full max-h-[800px]'}`}>
            ...
            {copied ? <CheckIcon className="w-5 h-5 text-brand-buy" /> : <CopyIcon className="w-5 h-5" />}
             ...
             {!isModal && onFullscreen && (
              <button 
                ...
                className="p-2 rounded-md bg-brand-primary hover:bg-brand-border transition-colors text-brand-muted hover:text-white focus:outline-none focus:ring-2 focus:ring-brand-accent"
