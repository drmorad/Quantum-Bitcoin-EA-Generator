...
const generateGridStrategyCode = (config: EAConfig): string => {
  const header = generateHeader('Grid', 'A trend-following grid EA for BTCUSD H1. See the NOTICE in the code.');
...
const generateSignalStrategyCode = (config: EAConfig): string => {
    const header = generateHeader('Signal', 'A signal-based EA for BTCUSD H1 using MA, RSI, and ATR. See the NOTICE in the code.');
